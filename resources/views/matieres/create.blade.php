...
@section('content')
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Créer une Nouvelle Matière</h1>
    ...
    <form action="{{ route('matieres.store') }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        <div class="mb-4">
            <label for="nom" class="block text-sm font-medium text-gray-700">Nom de la matière</label>
            <input type="text" name="nom" id="nom" value="{{ old('nom') }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="enseignant_ids[]" id="enseignant_ids" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                @foreach($enseignants as $enseignant)
                    <option value="{{ $enseignant->id }}" {{ in_array($enseignant->id, old('enseignant_ids', [])) ? 'selected' : '' }}>{{ $enseignant->nom }}</option>
                @endforeach
            </select>
        ...
        <div class="flex justify-end">
            ...
            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200">Créer</button>
        </div>
    </form>
@endsection